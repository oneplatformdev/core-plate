      ...
      >
        {element.texExpression.length > 0 ? (
          ...
          <div className="flex h-7 w-full items-center gap-2 whitespace-nowrap text-muted-foreground text-sm">
            ...
            <div>{defaultPlateMessages.addTexEquation}</div>
          </div>
        )}
      </div>
      {props.children}
...
export function EquationElementDocx(
  ...
  if (!element.texExpression || element.texExpression.length === 0) {
    return (
      <SlateElement {...props}>
        <p style={{ color: '#888', fontStyle: 'italic' }}>{defaultPlateMessages.emptyEquation}</p>
        {props.children}
      </SlateElement>
    );
  }
  ...
  if (!element.texExpression || element.texExpression.length === 0) {
    return (
      <SlateElement {...props} as="span">
        <span style={{ color: '#888', fontStyle: 'italic' }}>{defaultPlateMessages.inlineEquationPlaceholder}</span>
        {props.children}
      </SlateElement>
    );
  }
