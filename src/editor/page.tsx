...
import { createHtmlDocument } from '@op/modules/plate/lib/createHtmlDocument';
...
const createEditor = (editor: any) => createSlateEditor({
    plugins: [
        BaseColumnPlugin,
    ...
    value: editor.value.children,
});

export const SlateToHtmlBlock: React.FC<any> = (editor: any) => {
    const onPress = async () => {
        try {
            const editor2 = createEditor(editor);
            const editorHtml = await serializeHtml(editor2, {
                components,
    ...
    return <Button onClick={onPress}>Log</Button>;
