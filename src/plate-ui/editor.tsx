...
export type EditorProps = PlateContentProps &
  ...
  onChangeValues?: (state: PlateEditor["children"]) => void;
  isSimpleEditor?: boolean;
};

export const Editor = React.forwardRef<HTMLDivElement, EditorProps>(
  ({ className, disabled, focused, variant, isSimpleEditor, onChangeValues, ...props }, ref) => {
    const editorState = useEditorState();

    useEffect(() => {
      onChangeValues?.(editorState.children);
    }, [editorState.children, onChangeValues]);
    return (
      <PlateContent
        ref={ref}
        className={cn(
          ...
          className,
          isSimpleEditor && 'sm:px-6 sm:pt-3',
        )}
        disabled={disabled}
